...
void Card_Init(void)
{
	...
	card_motor.vpid.err=0;
	card_motor.vpid.last_err=0;
	card_motor.vpid.err_integration=0;
	card_motor.vpid.P_OUT=0;
	card_motor.vpid.I_OUT=0;
	card_motor.vpid.D_OUT=0;
	card_motor.vpid.PID_OUT=0;
}
...
{
		Vpid_Realize(card_motor, kp, ki, kd);
}
...
void Set_Card_Current()
{
	...
	//can发送数据帧
//	CAN1_Send_Card_Msg(current_msg);
}
/**
	* @brief  Card任务执行函数
	* @param void
