...
uint8_t RC_Chassis_Speed_Buffer[10];
uint8_t RX_Bufer[50];
uint32_t RecCount=0;
/**
...
{
	RC_Chassis_Speed_Buffer[0] = '&';		//开始标志位
	RC_Chassis_Speed_Buffer[1] = (speed1>>8);
	RC_Chassis_Speed_Buffer[2] = (speed1&0xff);
	RC_Chassis_Speed_Buffer[3] = (speed2>>8);
	RC_Chassis_Speed_Buffer[4] = (speed2&0xff);
	RC_Chassis_Speed_Buffer[5] = (speed3>>8);
	RC_Chassis_Speed_Buffer[6] = (speed3&0xff);
	RC_Chassis_Speed_Buffer[7] = (speed4>>8);
	RC_Chassis_Speed_Buffer[8] = (speed4&0xff);	
	RC_Chassis_Speed_Buffer[9] = ';';		//结束标志位
	
	HAL_UART_Transmit(&huart1,RC_Chassis_Speed_Buffer,10,0xff);
}
